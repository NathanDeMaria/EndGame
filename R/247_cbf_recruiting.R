...
# SPORTS247_CFB_FORMAT <- 'https://247sports.com/Season/{year}-Football/CompositeTeamRankings/?ViewPath=~%2FViews%2FSkyNet%2FInstitutionRanking%2F_SimpleSetForSeason.ascx&Page={page}&_=1565460248472'
...
.get_page_ratings <- function(year, page) {
  loginfo('Year: %s\tPage: %s', year, page)
  ...
  team_rows <- SPORTS247_CFB_FORMAT %>% str_glue(year = year) %>%
    ...
    html_nodes('li.rankings-page__list-item')

  tibble(
    name = team_rows %>% html_node('div.team') %>% html_text() %>% str_trim(),
    rating = team_rows %>% html_node('div.points') %>% html_text() %>% str_trim() %>% as.numeric(),
    year
  )
}
